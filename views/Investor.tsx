...
export const Investor: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 animate-fade-in space-y-16 pb-24 relative">
      {/* Top Header Controls */}
      <div className="flex justify-between items-center mb-4 no-print">
        <button onClick={onBack} className="flex items-center text-gray-500 hover:text-brand-black transition-colors group">
          <ArrowLeft className="w-5 h-5 mr-1 group-hover:-translate-x-1 transition-transform" /> Back
        </button>
        <button 
          onClick={onBack} 
          className="p-2 bg-gray-100 hover:bg-gray-200 text-gray-500 hover:text-brand-black rounded-full transition-all shadow-sm"
          title="Exit Investor View"
        >
          <X className="w-6 h-6" />
        </button>
      </div>
